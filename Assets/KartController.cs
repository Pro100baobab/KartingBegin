...
public class KartController : MonoBehaviour
{
    ...
    [SerializeField] private InputActionReference _moveActionRef;   // управление картингом
    ...
    private float _vLatFL, _vLatFR, _vLatRL, _vLatRR;    // v_lat для каждого колеса
    private float _fxFL, _fyFL, _fxFR, _fyFR, _fxRL, _fyRL, _fxRR, _fyRR;    // Fx, Fy для каждого колеса
    private float _slipRatioFL, _slipRatioFR, _slipRatioRL, _slipRatioRR;    // Slip ratio для каждого колеса
    ...
    /// <summary>
    /// Распределяем вес по осям через frontAxleShare: W_f​=frontShare⋅W,W_r​=(1−frontShare)⋅W
    /// Нормальная сила на каждое колесо:   N_{FL}​= N_{ FR}​=\frac{W_f}{ 2}​​,N_{RL}​= N_{ RR}​=\frac{W_r}{ 2}
    /// 
    ...
    private void ApplyWheelForces(Transform wheel, float normalForce, bool isDriven)
    {
        ...
        // 3) Боковая сила - используем разные значения для передней/задней осей
        ...
        // 6) Сохраняем данные о скольжении для телеметрии
        if (isDriven)
        {
    ...
    // Метод для применения ручного тормоза
    ...
    // Метод для расчета изначальных данных для распределения веса
    ...
    // Метод для получения данных о нажатии с клавиатуры
    ...
    private void OnGUI()
    {
        ...
        // Дополнительная информация о силах на колесах
    ...
    // Включаем/отключаем Actions в методе Enable/Disable
